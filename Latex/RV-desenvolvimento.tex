...
selecionado, resultando em um jogo intuitivo, divertido que capaz de transmitir 
os conceitos de lógicos e cognitivos relevantes.
...
\begin{figure}[h]
	...
	\legend{\fonteAP}
\end{figure}
\TODO{Ref. figura 2! "Queria que vocês explicassem as ideias dos jogos apresentados na Figura 2, acho que isso é importante. Precisa escrever também, porque a escolha final do jogo que vocês escolheram. Porque desse jogo e não dos demais pensados."}
...
um \textit{design} mais diretamente análogo ao mundo real, gerando um jogo no 
qual o jogador manipularia características de uma pequena configuração 
geológica tridimensional, de forma a manipulá-la para chegar a um determinado 
estado pré-definido.

Neste jogo, as crianças deparariam-se com um terreno digital composto por acidentes
...
a elevação, precipitação, movimentação do ar, entre outros. Este terreno 
seria subdivido em unidades cúbicas que poderiam ser manipuladas individualmente,
fornecendo uma resolução suficiente para que configurações diversas e 
interessantes pudessem ser construídas, mas sem exigir uma precisão incompatível 
com os dispositivos de entrada e saída escolhidos.
...
Foram então imaginados cenários que extrapolassem os usos básicos das 
...
movimentar massas de ar para jogar água contra blocos de terra, formando 
...
\TODO{" vocês falam das dificuldades, mas não falam o que fizeram para resolver 
	essa dificuldade. Falar de como foi resolvido no começo da secao da segunda iteracao"	}
...
nos levando a precisar reimaginar uma parte significativa da interação.
...
dados estruturados. Estes são, então, enviados a um celular que esteja rodando 
a lógica do jogo e fazendo as transformações necessárias para a saída de vídeo. 
...
\TODO{Concluir a tabela 2}

\TODO{Ver diferenca entre Tabela e Quadro. Lucy acha que isso é um quadro.}
% ---
\subsection{Escolha da arquitetura}\label{subsec-arquitetura-escolha}
% ---

Inicialmente, se optou pela alternativa do \textit{streaming} de video, visto que a 
...
Riftcat. Durante o desenvolvimento inicial, esta foi a alternativa utilizada, 
mesmo sendo necessário inicializar o software várias vezes, devido a 
bugs e falta de documentação do Rifcat.

Porém, descobriu-se posteriormente que para que esta alternativa funcionasse, 
...
Dado este contratempo, decidiu-se testar as outras duas alternativas para 
verificar suas viabilidades. 

A alternativa da conexão direta necessitava a obtenção da versão do serviço 
do \textit{Leap Motion} para o \textit{Android}, que não estava mais 
disponível no website da empresa. Afortunadamente, a co-orientadora 
...
Para implementar tais mudanças no código do plugin, foi necessário encontrar 
...
além de serem eficientes em questão de memória e objetos alocados, visto 
que rodam com frequencia alta e é o \textit{Unity} que gere a sua 
memória (ou seja, o Unity roda o coletor de lixo de tempos em tempos, 
algo que pode causar travamentos dependendo de quantos objetos 'mortos' existirem).


\TODO{[11:42 PM, 11/15/2016] Lucy TCC: falta o capítulo com a pesquisa                        
	[11:42 PM, 11/15/2016] Lucy TCC: falta o capítulo com os resultados da pesquisa                        
	[11:42 PM, 11/15/2016] Lucy TCC: falta a conclusão                        
	[11:43 PM, 11/15/2016] Lucy TCC: na introdução falta também os trabalhos relacionados.                        
	[11:43 PM, 11/15/2016] Lucy TCC: em nenhum momento vocês falam onde está sendo aplicado as habilidades no jogo de vocês.                        
	[11:43 PM, 11/15/2016] Lucy TCC: isso precisa ficar claro em algum lugar.}

\TODO{Apendice - Feito por nos. Anexo: Nao feito por nós.}

\TODO{Questionario foi validado, sim.}